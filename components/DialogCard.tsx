...
import postFeedback from "../hooks/postFeedback";
import { Answer } from "../providers/DialogProvider";

type Props = { data: Answer; type: "question" | "answer" };

function DialogCard({ data, type }: Props) {
  const [voteState, setVoteState] = React.useState<"up" | "down" | "none">(
    "none"
  );
  const isQuestion = type === "question";
  const { query, response, loading, id } = data;
  const text = (isQuestion ? query : response) ?? "sorry, we hit an error";
          ...
          <div>
            <svg
              onClick={() => {
                if (voteState === "up") {
                  setVoteState("none");
                  postFeedback({ method: "downvote", id });
                } else {
                  setVoteState("up");
                  postFeedback({ method: "upvote", id });
                }
              }}
